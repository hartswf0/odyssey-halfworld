...
window.kitVO={
  ...
  tryTurn(id,onEnd){
    ...
    const arm=()=>{if(tok!==voToken)return;
      try{voEl.currentTime=s.start;}catch(e){}
      voEl.play().catch(()=>{window.__voActive=false;});
      const iv=setInterval(()=>{if(tok!==voToken){clearInterval(iv);return;}
        if(voEl.paused||voEl.currentTime>=s.start+s.dur-.05){clearInterval(iv);if(tok===voToken)voStop();}},120);};
    if(voEl.readyState>=1)arm();else voEl.onloadedmetadata=arm;
    return true;
  },
  playScene(sceneId){
    if(voDisabled)return false;
    const m=VOMAN&&VOMAN[sceneId];if(!m)return false;
    voStop();const tok=++voToken;
    voEl.src=m.file;window.__voActive=true;
    document.querySelectorAll("audio").forEach(a=>{
      if(a!==voEl&&!a.paused){voDuck.push([a,a.volume]);a.volume=Math.min(a.volume,.2);}});
    voEl.onloadedmetadata=()=>{if(tok===voToken){try{voEl.currentTime=0;}catch(e){}voEl.play().catch(()=>{});}};
